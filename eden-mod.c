...
int main(int argc, char **argv) {
    ...
    memset(hud_blob, 0xcc, sizeof hud_blob);
    size = (long)(_binary_hud_payload_bin_end -
                  _binary_hud_payload_bin_start);
    if (size <= 0 || size > (long)sizeof hud_blob) {
        puts("Embedded HUD payload is empty or exceeds 8192 bytes.");
        return 1;
    }
    ...
    hud_hook[0] = 0xe9;
    rel = (int32_t)(UINT64_C(0x1446c7000) -
                    (UINT64_C(0x14009a11d) + 5));
