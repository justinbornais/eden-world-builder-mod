...
- HUD squares show active modes: green for flight, red for noclip, repeat-rate colors, vibrant pink for replace mode, and yellow/orange for auto-fill armed/waiting states.
...
- Terrain is loaded in larger, less-frequent batches.
...
```powershell
gcc -O2 -Wall -Wextra -o eden-mod.exe eden-mod.c
.\eden-mod.exe "Eden - World Builder.exe" "Eden - Modded Test.exe"
```

Always keep the original EXE unchanged and provide a new output filename. Test new builds under another name.
